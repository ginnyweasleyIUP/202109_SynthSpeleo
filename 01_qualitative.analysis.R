...
file_path1 = '/home/ldap-server/ginnyweasley/07_R_Code/202109_SynthSpeleo/test'
file_path2='/home/ldap-server/ginnyweasley/07_R_Code/202109_SynthSpeleo/test/summary'
p.min=0.1
p.max=300
p.n=5
n=10
h.length=c(1000,2000,500)
...
quanAna <- function(p.min = 0.1, p.max=1,p.n=5,t.min=0,t.max=10000,t.n=4,d.min=0,d.max=1000,h.length=c(2000,3000,500),file_path1 ='~/synSpeleo/test/', file_path2 = '~/synSpeleo/test/summary' ,rate=3,t.res=2, nsamp=15,n=100){
  ...
  for(k in 0:length(h.length)){
    ...
    for (j in prec){
      ...
      for (i in 1:n) {
        #print(i)
        print(paste("k:",k,"j:",j,"i:",i," Were in run ", number.runs, "of", (length(h.n)+1)*(length(prec))*n))
        ...
        run_SISAL_chrono(speleo_new, file_path2,paste(l,'-',j,'-',i,sep = '') , hiatus)
...
synSpeleo_chrono <- quanAna(file_path1 = '/home/ldap-server/ginnyweasley/07_R_Code/202109_SynthSpeleo/test',
                            file_path2='/home/ldap-server/ginnyweasley/07_R_Code/202109_SynthSpeleo/test/summary', p.min=0.1,p.max=300,p.n=5, n=3,h.length=c(1000,2000,500))

#save(synSpeleo_chrono, file = '/stacycode/ariana/synSpeleo/summary/synSpeleo_chrono.RData')
save(synSpeleo_chrono, file = '/home/ldap-server/ginnyweasley/07_R_Code/202109_SynthSpeleo/test/synSpeleo_chrono.RData')


############## HERE #########################################################################

#72=n*(p.n+1)*(lenght(h.length)+1)
names <- rep(NA_real_,72)
counter <- 0
for(k in 0:3){
  ...
  for (j in prec){
    ...
    for (i in 1:3) {
      ...
      counter <- counter+1
      if(k==0){
        name <- paste('/home/ldap-server/ginnyweasley/07_R_Code/202109_SynthSpeleo/test/summary/chrono-',k,'-',j,'-',i,'.RData',sep='')
        names[counter] <- name
      } else {
        name <- paste('/home/ldap-server/ginnyweasley/07_R_Code/202109_SynthSpeleo/test/summary/chrono-',k,'-',j,'-',i,'.RData',sep='')
        names[counter] <- name
...
prec_hiatus <- tibble(entity_id = character(),hiatus_nr = numeric(),prec = numeric(), iteration = numeric() ,lr = numeric(),li=numeric(),copRa=numeric(),bacon=numeric(),stalage=numeric())
for(name in names){
...
mean(prec_hiatus_new$bchron.prec)
...
if(T){
  pdf('Plots/FirstCarlaTest.pdf',8,12)
