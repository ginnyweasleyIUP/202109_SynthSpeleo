...
run <- run %>% filter(entity_id > 510) %>% arrange(., entity_id)  # 330 Bchron geht nicht - > 2: ab 331 bis einschliesslich 377, 378 not working for Bchron (3)
                                                                  #506Bchron geht nicht; 4 -> ab 507; 5: ab 511 (510 geht nicht)
runFile <- tibble(entity_id = run$entity_id, Bacon = rep(T, length(run$entity_id)), Bchron = rep(T, length(run$entity_id)), 
                  ...
                  working_directory = rep("/home/ariana/Documents/v2", length(run$entity_id)), 
                  wd = rep('/home/ariana/SISAL Data/interim_v2_csv_20190812', length(run$entity_id)))
