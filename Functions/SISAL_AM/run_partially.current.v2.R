...
run<- read.csv('~/Documents/partially.current.v2/run.csv', header = T)
...
run_SISAL_chrono <- function(entid, data,working_directory, bacon, bchron, stalage, linInterp, copRa, linReg,j) {
  ...
  write.csv(runFile, 'runFile_pc.csv', row.names = F, col.names = T)
  
  print(paste(file_name, 'done'))
}

load_SISAL('', runFile) # v1b
setwd("~/Documents/v2_90ci/")
write.csv(runFile, 'runFile_partially.current.csv', row.names = F, col.names = T)