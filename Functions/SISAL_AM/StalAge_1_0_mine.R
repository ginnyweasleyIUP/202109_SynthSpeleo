...
#### modified by CARLA ROESCH
# --------------------------------------------------------------------------------------------------####


StalAge <- function(working_directory = getwd()){
  #source(file = "StalAge_1_0_mod.R")
  ...
  depth_sample <- depths$dft
  StalAge_age <- fit$StalAge_age
  StalAge_age_uncert_pos <- fit$StalAge_age_uncert_pos
  StalAge_age_uncert_neg <- fit$StalAge_age_uncert_neg
  
  chronology <- as.data.frame(cbind(sample_id,StalAge_age, StalAge_age_uncert_pos - StalAge_age, StalAge_age - StalAge_age_uncert_neg))
  colnames(chronology) <- c('sample_id','StalAge_age', 'StalAge_age_uncert_pos', 'StalAge_age_uncert_neg')
  #colnames(chronology) <- c(sample_id,stal_age_age, stal_age_age_uncert_pos, stal_age_age_uncert_neg)
  #mat_chronology <- as.matrix(chronology)
  #colnames(mat_chronology) <- NULL
  
  
  write.csv(cbind(sample_id,depth_sample, StalAge_age, StalAge_age_uncert_pos, StalAge_age_uncert_neg), file="StalAge_results.csv", 
            col.names = T, row.names = F)
  write.table(chronology, file="StalAge_chronology.csv", col.names = T,sep = ',' ,row.names = F)
  write.csv(Daten, file = 'StalAge_date_used.csv', col.names = T, row.names = F)
  
  pdf('final_age_model.pdf', 6,4)
  matplot(x = StalAge_age, y= depth_sample, col = 'black', lty = 1, type = 'l', lwd = 1, 
          ylim = c(max(depth_sample),0), xlab = 'Age [kyrs BP]', ylab = 'Depth from top [mm]')
  lines(x=StalAge_age_uncert_pos,y=depth_sample, lty = 2, col = 'red')
  lines(x=StalAge_age_uncert_neg,y = depth_sample, lty = 2, col = 'red')
  ...
  #Speichern der Grafiken als pdf
  
  #print('#----------SAVE GRAPHICS----------------#')
  #working_directory <- getwd()
  #setwd(working_directory)
  #dir.create(file.path(working_directory,'graphics'))
  #setwd(file.path(working_directory,'/graphics'))
  
  
  #for (i in 2:8) {
  
  #dev.set(which = i)
  #dev.copy2pdf(file = paste("Fig._", i-1, ".pdf", sep = ""))
  #dev.copy2pdf(file = paste("Fig._", i-1, "_mean_asymm.pdf", sep = ""))
  #dev.copy2pdf(file = paste("Fig._", i-1, "_larger_asymm.pdf", sep = ""))
...
age_model<-function(Daten, x_raw, Daten_orig, q1 = 0.025, q2 = 0.975) {
...
results3[i,2]<-quantile(results2[i,,], probs=q2, na.rm=TRUE, names=FALSE)-results3[i,1]
results3[i,3]<-results3[i,1]-quantile(results2[i,,], probs=q1, na.rm=TRUE, names=FALSE)
...
results4[i,2]<-quantile(y[,i], probs=q2, na.rm=TRUE, names=FALSE)-results4[i,1]
results4[i,3]<-results4[i,1]-quantile(y[,i], probs=q1, na.rm=TRUE, names=FALSE)
